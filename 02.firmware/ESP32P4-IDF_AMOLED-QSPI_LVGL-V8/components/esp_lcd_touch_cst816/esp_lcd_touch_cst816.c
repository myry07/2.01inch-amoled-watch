...
esp_err_t esp_lcd_touch_new_i2c_cst816(const esp_lcd_panel_io_handle_t io, const esp_lcd_touch_config_t *config, esp_lcd_touch_handle_t *tp)
{
    ...
    if (cst816->config.rst_gpio_num != GPIO_NUM_NC) {
        const gpio_config_t rst_gpio_config = {
            .mode = GPIO_MODE_OUTPUT,
            .pin_bit_mask = BIT64(cst816->config.rst_gpio_num)
        };
    ...
    /* Read product id */
    ESP_GOTO_ON_ERROR(read_id(cst816), err, TAG, "Read version failed");
    *tp = cst816;

    ESP_LOGI(TAG, "LCD touch panel create success, version: %d.%d.%d", ESP_LCD_TOUCH_CST816_VER_MAJOR, ESP_LCD_TOUCH_CST816_VER_MINOR,
             ESP_LCD_TOUCH_CST816_VER_PATCH);
